...
PVOID GetGuardedRegion(DWORD FirstPointer)
{
	PSYSTEM_BIGPOOL_INFORMATION pPoolInfo = 0;
	NTSTATUS ntstatus = STATUS_SUCCESS;
	ULONG infoLen = 0;

	ntstatus = ZwQuerySystemInformation(system_bigpool_information, &infoLen, 0, &infoLen);
	while (ntstatus == STATUS_INFO_LENGTH_MISMATCH)
	{
		...
		ntstatus = ZwQuerySystemInformation(system_bigpool_information, pPoolInfo, infoLen, &infoLen);
	}
	...
	if (pPoolInfo)
	{
		for (ULONG i = 0; i < pPoolInfo->Count; ++i)
		{
			...
			{
				ULONG Rsh_x24 = (*(uintptr_t*)((PBYTE)VirtualAddress + FirstPointer) >> 0x24);
